...
function interestBlockAnimate(elemClass) {
  ...
  if (block) {
    this.setTimeout(() => {
      // console.log("unLockedDocumentWidth: ", unLockedDocumentWidth);
      ...
      const intUpLegRightStep = block.querySelector(
        ".interest__man-up-leg-r-step"
      );
      ...
      function isBeforeIntAction() {
        ...
        return currentIntSceneBottom > intLandingPoint ? true : false;
      }
      ...
      function isAfterIntAction() {
        ...
        return currentIntSceneBottom < intExitPoint ? true : false;
      }
      ...
      window.addEventListener("scroll", function () {
        ...
        // intMeeting()
        if (
          (currentIntSceneBottom < intExitPoint && animateHandShakeId) ||
          (currentIntTopCloudTop > intLandingPoint && animateHandShakeId)
        ) {
        ...
        if (
          (currentIntSceneBottom > intExitPoint &&
            currentIntSceneBottom < intLandingPoint &&
            canHandShake &&
            !animateHandShakeId) ||
          (currentIntTopCloudTop > intExitPoint &&
            currentIntTopCloudTop < intLandingPoint &&
            canHandShake &&
            !animateHandShakeId)
        ) {
      ...
      function intLanding() {
        ...
        if (
          (isInIntLandingRange() && !isLanded) ||
          (isInIntMeetingRange() && !isLanded) ||
          (isInIntHandShakingRange() && !isLanded)
        ) {
          ...
          setTimeout(() => {
            ...
            let targetLandingScroll =
              landingScrollStart + (currentIntSceneBottom - intMeetingPoint);
            ...
            animateScrollLanding({
              ...
              draw(progress) {
                window.scrollTo(
                  0,
                  landingScrollStart -
                    (landingScrollStart - targetLandingScroll) * progress
                );
                intFlyMan.style.transform = `translate(${-10 * progress}%, ${
                  134 * progress
                }%) rotate(${21 * progress}deg)`;
        ...
        if (canMeet && !isMeetingAnimated) {
          ...
          animateScrollMeeting({
            ...
            draw(progress) {
              ...
              intBagFire.style.opacity = `${
                1.5 * progress > 1 ? 0 : 1 - 1.5 * progress
              }`;
              ...
              intUpMan.style.transform = `translate(${44 * progress}%, ${
                -23 * progress
              }%)`;
              ...
              intUpLegRight.style.opacity = `${
                5 * progress > 1 ? 0 : 1 - 5 * progress
              }`;

              // intUpLegRightStep.style.transform = `translate(0%, ${-100 + 100 * progress}%)`
              intUpLegRightStep.style.opacity = `${
                3 * progress <= 2 ? 0 : 3 * progress - 2
              }`;

              intArrowSm.style.transform = `translate(${75 * progress}%, ${
                -81 * progress
              }%)`;
              intArrowSm.style.opacity = `${0.5 + 0.5 * progress}`;
              intArrowBlue.style.transform = `translate(${101 * progress}%, ${
                -97.4 * progress
              }%)`;
              intArrowBlue.style.opacity = `${0.5 + 0.5 * progress}`;
              intArrowMd.style.transform = `translate(${67 * progress}%, ${
                -59 * progress
              }%)`;
              intArrowMd.style.opacity = `${0.5 + 0.5 * progress}`;
      ...
      let animateHandShakeOptions = {
        ...
        draw(progress, progress2) {
          intTopCloud1.style.transform = `translate(${12 * progress}%, ${
            -3 * progress
          }%)`;
          intTopCloud2.style.transform = `translate(${5 * progress}%, ${
            0 * progress
          }%)`;
          intTopCloud3.style.transform = `translate(${18 * progress}%, ${
            0 * progress
          }%)`;
          intTpCloud4.style.transform = `translate(${-16 * progress}%, ${
            5 * progress
          }%) rotate(${-180}deg)`;

          intTopCloud.style.transform = `translate(${-30 * progress}%, ${
            0 * progress
          }%)`;
          intTpCloud6.style.transform = `translate(${-40 * progress}%, ${
            -10 * progress
          }%)`;

          intMdCloud1.style.transform = `translate(${60 * progress}%, ${
            -10 * progress
          }%)`;
          intMdCloud2.style.transform = `translate(${12 * progress}%, ${
            -5 * progress
          }%)`;
          intMdCloud3.style.transform = `translate(${-50 * progress}%, ${
            25 * progress
          }%)`;
          intMdCloud4.style.transform = `translate(${-35 * progress}%, ${
            -15 * progress
          }%)`;
          intMdCloud5.style.transform = `translate(${-35 * progress}%, ${
            0 * progress
          }%)`;

          // intFlyManHand.style.transformOrigin = `75% 75%`
          intFlyManHand.style.transform = `translateY(${
            2 * progress2
          }%) rotate(${46 - 10 * progress2}deg)`;
          intUpHand.style.transform = `translateX(${0}%) rotate(${
            -57 + 10 * progress2
          }deg)`;
        },
      ...
      function animateHandShake({ timing, timing2, draw, duration }) {
        ...
        requestAnimationFrame(function animateHandShake(time) {
          ...
          let timeFraction = (time - start) / duration;
          let timeFraction2 = Math.abs(
            1 - Math.abs(1 - Math.abs(1 - (4 * (time - start)) / duration))
          );
