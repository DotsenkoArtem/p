...
function expBlockAnimate(elemClass) {
  ...
  if (block) {
    this.setTimeout(() => {
      ...
      expTextAnimItems.push(
        block.querySelector(".selection-text"),
        block.querySelector(".selection-title")
      );
      ...
      function expAnimate() {
        ...
        if (unLockedDocumentWidth > 1000) {
          expAnimStartPoint =
            window.innerHeight - block.getBoundingClientRect().height;
          expAnimStopPoint = block.getBoundingClientRect().height + 100;
          if (
            (currentBlockTop <= expAnimStartPoint &&
              currentBlockBottom > expAnimStartPoint &&
              scrollDirection < 0) ||
            (currentBlockBottom >= expAnimStopPoint &&
              currentBlockTop < expAnimStopPoint &&
              scrollDirection > 0)
          ) {
            expAnimItems.forEach((item) => {
        ...
        if (unLockedDocumentWidth <= 1000) {
          ...
          expTextAnimItems.forEach((item) => {
            if (
              (item.getBoundingClientRect().top <=
                window.innerHeight - item.getBoundingClientRect().height &&
                item.getBoundingClientRect().bottom >
                  window.innerHeight - item.getBoundingClientRect().height &&
                scrollDirection < 0) ||
              (item.getBoundingClientRect().bottom >=
                item.getBoundingClientRect().height + 100 &&
                item.getBoundingClientRect().top <
                  item.getBoundingClientRect().height + 100 &&
                scrollDirection > 0)
            ) {
          ...
          expSceneItems.forEach((item) => {
            if (
              (expScene.getBoundingClientRect().top <=
                window.innerHeight - expScene.getBoundingClientRect().height &&
                expScene.getBoundingClientRect().bottom >
                  window.innerHeight -
                    expScene.getBoundingClientRect().height &&
                scrollDirection < 0) ||
              (expScene.getBoundingClientRect().bottom >=
                expScene.getBoundingClientRect().height + 100 &&
                expScene.getBoundingClientRect().top <
                  expScene.getBoundingClientRect().height + 100 &&
                scrollDirection > 0)
            ) {
